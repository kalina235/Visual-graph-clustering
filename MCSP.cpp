...
void MCSP::FindCoordinates()
{
	...
	{
		m_finalCoordinates.push_back(make_pair(xCoordinates(index),yCoordinates(index)));
	}
}
...
void MCSP::PrintCoordinates(std::string fileName)
{
	...
	printf("{%lf,%lf}", m_finalCoordinates[0].first, m_finalCoordinates[0].second);
	for(auto index = 1; index < m_matSize; index++)
	{
		printf(",{%lf,%lf}", m_finalCoordinates[index].first, m_finalCoordinates[index].second);
	}
	fclose(stdout);
}
